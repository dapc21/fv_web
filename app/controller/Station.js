 ...
 * 
 * @author John Jimenez
 * @class LoadPrincipal.controller.Station
 * @extends LoadPrincipal.controller.Core
 * Class to manage the Station module events
 * 
 */
        ...
        this.control(
            {
                 ...
                 * Clear filters
                 */
                'AliasStationFilter button[action=clearFilters]': {
                    click: this.clearFilters
                },
                'AliasStationFilter button[action=clearFilter]': {
                    click: this.clearFilter
                },
                'AliasStationList button[action=clearFilter]': {
                    click: this.clearFilter
                },
                'AliasStationList button[action=StationListCreate]': {
                    click: this.openModalWindowLocal
                },
                'AliasStationList button[action=StationListDelete]': {
                    click: this.requestExample
                },
                /**
                 * Filters
                 */
                ...
                'AliasStationFilter combo[id=StationFilterName]': {
                    beforequery: this.autofill,
                    select: this.multiSearch
                },
                'AliasStationFilter combo[id=StationFilterAction]': {
                    beforequery: this.autofill,
                    select: this.multiSearch
                },
                'AliasStationFilter combo[id=columnas2]': {
                    select: this.columnSelect
                },
                'AliasStationFilter numberfield[id=value1]': {
                    keyup: this.validateValue1
                },
                'AliasStationFilter numberfield[id=valueRange1]': {
                    keyup: this.validateRange
                },
                'AliasStationFilter numberfield[id=valueRange2]': {
                    keyup: this.validateRange
                },
                'AliasStationFilter checkbox[id=allEstratos]': {
                ...
                '#map': {
                    click: this.biuldStation
                },
                
                'AliasStationFilter combo[id=StationFilterSection]': {
                    beforequery: this.autofill,
                    select: this.multiSearch
                },
                'AliasStationFilter datefield[id=StationFilterStart]': {
                    change: this.SelectFilterDateStart
                },
                'AliasStationFilter datefield[id=StationFilterEnd]': {
                    change: this.SelectFilterDateEnd
                },
                'AliasStationFilter checkbox[id=StationFilterDeleted]': {
                 ...
                 * Excel
                 */
                'AliasStationList button[action=exportXls]': {
...
//                                              + lonlat.lon + " E");
        ...
        var map = new OpenLayers.Station({
                controls: [
...
//            );
            ...
            mappanel = Ext.create('GeoExt.panel.Station', {
...
//                win.destroy();
    ...
    openModalWindowLocal: function(button){
        alert('ingresa Station controller')
//        var obj = this;
//        this.loadScript('app/controller/MapComponent.js',function(){
//            
//            
//        })
//        obj.loadComponent('MapComponent');

        try {

            this.getController('MapComponent').openModalWindowLocal();

        }
        catch (ex) {
            Ext.Error.raise('Application component error: ' +
                    'Controller MapComponent is unavailable');
        }
    },
...
//        });
        Ext.Ajax.request({
            url: 'http://192.168.1.114/csvprocessor/public/objects',
//            url: 'http://www.w3schools.com/cssref/css3_pr_text-shadow.asp',
            type: 'rest',
            method: 'GET',
//            cors: true,
