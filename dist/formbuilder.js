...
(function() {
  ...
  FormbuilderModel = (function(_super) {
    ...
    FormbuilderModel.prototype.is_input = function() {
      return Formbuilder.inputFields[this.get(Formbuilder.options.mappings.XTYPE)] != null;
    };
  ...
  ViewFieldView = (function(_super) {
    ...
    ViewFieldView.prototype.render = function() {
      this.$el.addClass('response-field-' + this.model.get(Formbuilder.options.mappings.XTYPE)).data('cid', this.model.cid).html(Formbuilder.templates["view/base" + (!this.model.is_input() ? '_non_input' : '')]({
        rf: this.model
      }));
      return this;
    };
    ...
    ViewFieldView.prototype.clear = function(e) {
      ...
      e.stopPropagation();
      cb = function() {
        /* Se destruye el objeto padre del array questions */
        _this.parentView.destroyParent(_this.model.attributes.cid);
  ...
  EditFieldView = (function(_super) {
    ...
    EditFieldView.prototype.defaultUpdated = function(e) {
      var $el;
      $el = $(e.currentTarget);
      if (this.model.get(Formbuilder.options.mappings.XTYPE) !== 'checkboxfield') {
    ...
    EditFieldView.prototype.addName = function(e) {
      ...
      this.model.set(Formbuilder.options.mappings.OPTIONS, newOption);
      this.model.trigger("change:" + Formbuilder.options.mappings.OPTIONS);
      return this.forceRender();
    };
    ...
    EditFieldView.prototype.addOptionCombo = function(e) {
      ...
      $el = $(e.currentTarget);
      i = this.$el.find('.option').index($el.closest('.option'));
      options = this.model.get(Formbuilder.options.mappings.OPTIONSCOMBO) || [];
      var len = (options.length)+1;
      var newval = len.toString();
      newOption = {
        id         : 'option'+newval,
      ...
      this.model.set(Formbuilder.options.mappings.OPTIONSCOMBO, options);
      this.model.trigger("change:" + Formbuilder.options.mappings.OPTIONSCOMBO);
      return this.forceRender();
    };
    ...
    EditFieldView.prototype.updateRegexDatefield = function() {
      ...
      this.model.set(Formbuilder.options.mappings.EMPTYTEXT, emptyText);
      this.model.set(Formbuilder.options.mappings.REGEX, regex);
      this.model.set(Formbuilder.options.mappings.REGEXTEXT, regexText);
      this.model.trigger("change:" + Formbuilder.options.mappings.EMPTYTEXT);
      this.model.trigger("change:" + Formbuilder.options.mappings.REGEX);
      this.model.trigger("change:" + Formbuilder.options.mappings.REGEXTEXT);
      return this.forceRender();
    };
    ...
    EditFieldView.prototype.updateRegexTimefield = function() {
      ...
      this.model.set(Formbuilder.options.mappings.EMPTYTEXT, empText);
      this.model.set(Formbuilder.options.mappings.REGEX, regex);
      this.model.set(Formbuilder.options.mappings.REGEXTEXT, regexText);
      this.model.set(Formbuilder.options.mappings.SUBMITFORMAT, submitFormat);
      this.model.trigger("change:" + Formbuilder.options.mappings.EMPTYTEXT);
      this.model.trigger("change:" + Formbuilder.options.mappings.REGEX);
      this.model.trigger("change:" + Formbuilder.options.mappings.REGEXTEXT);
      this.model.trigger("change:" + Formbuilder.options.mappings.SUBMITFORMAT);
      return this.forceRender();
    };
    ...
    EditFieldView.prototype.updateFileExtensions = function() {
      ...
      this.model.set(Formbuilder.options.mappings.FILEEXTENSIONS, fileExtensions);
      this.model.trigger("change:" + Formbuilder.options.mappings.FILEEXTENSIONS);
      return this.forceRender();
    };
  ...
  BuilderView = (function(_super) {
    ...
    BuilderView.prototype.createAndShowEditView = function(model) {
      ...
      this.addParent(newModel.attributes);
      console.log('FormBuilder Selected');
      return this;
    };
    ...
    BuilderView.prototype.setOptions = function(model) {
      ...
      $(".checked option[value='" + attrs.configuration.checked + "']").attr("selected","selected");
      if(attrs.xtype == 'gridfield') {
        $('#contentGridfield').attr("id","contentGridfield-"+attrs.cid); /* Actualizar id a la capa gridfield */
        $('#buttonModalFormbuilder').val(attrs.cid); /* Asignamos el cid al value del botón que abre la ventana modal */
      }
    ...
    BuilderView.prototype.destroyParent = function(cid) {
      ...
      this.checkAndDestroyChildren(questions, cid);
    };

    BuilderView.prototype.checkAndDestroyChildren = function(questions, cid) {
      var qchlength = Object.keys(questions);
      var band = 0;
      if (qchlength.length > 0) {
        for(var j = 0, len = qchlength.length; j < len; j++) {
          if (questions[j].id_parent) {
            if (questions[j].id_parent === cid) {
      ...
      if (band == 1) this.checkAndDestroyChildren(questions, cid);
      else this.updateParent(questions);
    };

    BuilderView.prototype.addParent = function(newParent) {
      var selected = Ext.getCmp(controller+'GridSection').getSelectionModel().getSelection();
      var record = selected[0];
      var name = record.get('name');
      var id_form = record.get('id_form');
      var questions = record.get('questions');
      var newQuestions = questions.concat(newParent);
      return this.handleFormUpdate();
    };

    BuilderView.prototype.updateParent = function(questions) {
      var selected = Ext.getCmp(controller+'GridSection').getSelectionModel().getSelection();
      var record = selected[0];
    ...
    BuilderView.prototype.getPayload = function() {
      ...
      if(qlength > 1 && sw == 1) {
        var parents = this.sortPayloadParents(qlength, questions);
        for (var pt=0; pt < qlength; pt++) {
          if (questions[pt].id_parent) {
            children.push(questions[pt]);
          }
        }
        questions = [];
        questions = children.concat(parents);
      }
      return questions;
    };

    BuilderView.prototype.sortPayloadParents = function(qlength, questions) {
      var originalPayload = fb.mainView.collection.toJSON();
      var payload = [];
  ...
  Formbuilder = (function() {
    Formbuilder.helpers = {
      defaultFieldAttrs: function(field_type) {
        var attrs, _base;
        attrs = {};
        attrs[Formbuilder.options.mappings.ORDER] = 'order';
        attrs[Formbuilder.options.mappings.HASHTAG] = 'Hashtag';
        attrs[Formbuilder.options.mappings.HELPTEXT] = 'Helptext';
        attrs[Formbuilder.options.mappings.XTYPE] = field_type;
        attrs[Formbuilder.options.mappings.FIELDLABEL] = 'Title';
        attrs[Formbuilder.options.mappings.EMPTYTEXT] = 'Default text...';
        attrs[Formbuilder.options.mappings.BLANKTEXT] = 'This field is required';
        attrs[Formbuilder.options.mappings.ALLOWBLANK] = false;
        attrs[Formbuilder.options.mappings.HIDDEN] = false;
        attrs[Formbuilder.options.mappings.HIDEEMPTYLABEL] = true;
        attrs[Formbuilder.options.mappings.HIDELABEL] = false;
        attrs[Formbuilder.options.mappings.READONLY] = false;
        attrs['configuration'] = {};
        //attrs.configuration[Formbuilder.options.mappings.SECTION] = true;
    ...
    Formbuilder.jump = function(name, opts) {
...
(function() {
  Formbuilder.registerField('textfield', {
    order: 1,
    view: "<input type='text' class='rf-size-large' placeholder='<%= rf.get(Formbuilder.options.mappings.EMPTYTEXT) %>' />",
    edit: "\n<%= Formbuilder.templates['edit/emptytext']() %>\n<%= Formbuilder.templates['edit/blanktext']() %>\n<%= Formbuilder.templates['edit/allowblank']() %>\n<%= Formbuilder.templates['edit/hidden']() %>\n<%= Formbuilder.templates['edit/hideemptylabel']() %>\n<%= Formbuilder.templates['edit/hidelabel']() %>\n<%= Formbuilder.templates['edit/readonly']() %>\n<%= Formbuilder.templates['edit/value']() %>\n<%= Formbuilder.templates['edit/regex']() %>\n<%= Formbuilder.templates['edit/regextext']() %>\n<%= Formbuilder.templates['edit/vtype']() %>\n<%= Formbuilder.templates['edit/vtypetext']() %>\n<%= Formbuilder.templates['edit/minlengthtext']() %>\n<%= Formbuilder.templates['edit/maxlengthtext']() %>\n<%= Formbuilder.templates['edit/validateblank']() %>\n<%= Formbuilder.templates['edit/min_max_length']() %>",
...
(function() {
  Formbuilder.registerField('textareafield', {
    order: 2,
    view: "<textarea placeholder='<%= rf.get(Formbuilder.options.mappings.EMPTYTEXT) %>' class='rf-size-large'></textarea>",
    edit: "\n<%= Formbuilder.templates['edit/emptytext']() %>\n<%= Formbuilder.templates['edit/blanktext']() %>\n<%= Formbuilder.templates['edit/allowblank']() %>\n<%= Formbuilder.templates['edit/hidden']() %>\n<%= Formbuilder.templates['edit/hideemptylabel']() %>\n<%= Formbuilder.templates['edit/hidelabel']() %>\n<%= Formbuilder.templates['edit/readonly']() %>\n<%= Formbuilder.templates['edit/value']() %>\n<%= Formbuilder.templates['edit/regex']() %>\n<%= Formbuilder.templates['edit/regextext']() %>\n<%= Formbuilder.templates['edit/vtype']() %>\n<%= Formbuilder.templates['edit/vtypetext']() %>\n<%= Formbuilder.templates['edit/minlengthtext']() %>\n<%= Formbuilder.templates['edit/maxlengthtext']() %>\n<%= Formbuilder.templates['edit/validateblank']() %>\n<%= Formbuilder.templates['edit/cols']() %>\n<%= Formbuilder.templates['edit/rows']() %>\n<%= Formbuilder.templates['edit/min_max_length']() %>",
...
(function() {
  Formbuilder.registerField('checkboxfield', {
    order: 3,
    view: "\n<label class='section-name'>\n<input type='checkbox' class='fb-checkbox' <%= rf.get(Formbuilder.options.mappings.CHECKED) %> />&nbsp;&nbsp;<%= rf.get(Formbuilder.options.mappings.BOXLABEL) %></label>\n",
    edit: "<%= Formbuilder.templates['edit/partialcheckbox']() %>",
...
(function() {
  Formbuilder.registerField('fieldcontainer', {
    order: 4,
    view: "<% for (i in (rf.get(Formbuilder.options.mappings.OPTIONS) || [])) { %>\n  <div>\n    <label class='fb-option'>\n      <input type='radio' <%= rf.get(Formbuilder.options.mappings.OPTIONS)[i].checked && 'checked' %> onclick=\"javascript: return false;\" />\n      <%= rf.get(Formbuilder.options.mappings.OPTIONS)[i].boxLabel %>\n    </label>\n  </div>\n<% } %>\n\n<% if (rf.get(Formbuilder.options.mappings.INCLUDE_OTHER)) { %>\n  <div class='other-option'>\n    <label class='fb-option'>\n      <input type='radio' />\n      Other\n    </label>\n\n    <input type='text' />\n  </div>\n<% } %>",
...
(function() {
  Formbuilder.registerField('combobox', {
    order: 5,
    view: "<select placeholder='<%= rf.get(Formbuilder.options.mappings.EMPTYTEXT) %>' >\n  <% if (rf.get(Formbuilder.options.mappings.INCLUDE_BLANK)) { %>\n    <option value=''></option>\n  <% } %>\n\n  <% for (i in (rf.get(Formbuilder.options.mappings.OPTIONS) || [])) { %>\n    <option <%= rf.get(Formbuilder.options.mappings.OPTIONS)[i].checked && 'selected' %>>\n      <%= rf.get(Formbuilder.options.mappings.OPTIONS)[i].label %>\n    </option>\n  <% } %>\n</select>",
    edit: "\n<%= Formbuilder.templates['edit/emptytext']() %>\n<%= Formbuilder.templates['edit/blanktext']() %>\n<%= Formbuilder.templates['edit/allowblank']() %>\n<%= Formbuilder.templates['edit/displayfield']() %>\n<%= Formbuilder.templates['edit/valuefield']() %>\n<%= Formbuilder.templates['edit/pagesize']() %>\n<%= Formbuilder.templates['edit/typeahead']() %>\n<%= Formbuilder.templates['edit/forceselection']() %>\n<%= Formbuilder.templates['edit/triggeraction']() %>\n<%= Formbuilder.templates['edit/optionscombo']() %>",
...
(function() {
  Formbuilder.registerField('numberfield', {
    order: 6,
    view: "<input placeholder='<%= rf.get(Formbuilder.options.mappings.EMPTYTEXT) %>' class='rf-size-large' type='text' />\n<% if (units = rf.get(Formbuilder.options.mappings.UNITS)) { %>\n  <%= units %>\n<% } %>",
    edit: "\n<%= Formbuilder.templates['edit/emptytext']() %>\n<%= Formbuilder.templates['edit/blanktext']() %>\n<%= Formbuilder.templates['edit/allowblank']() %>\n<%= Formbuilder.templates['edit/hidden']() %>\n<%= Formbuilder.templates['edit/hideemptylabel']() %>\n<%= Formbuilder.templates['edit/hidelabel']() %>\n<%= Formbuilder.templates['edit/readonly']() %>\n<%= Formbuilder.templates['edit/value']() %>\n<%= Formbuilder.templates['edit/regex']() %>\n<%= Formbuilder.templates['edit/regextext']() %>\n<%= Formbuilder.templates['edit/vtype']() %>\n<%= Formbuilder.templates['edit/vtypetext']() %>\n<%= Formbuilder.templates['edit/maxtext']() %>\n<%= Formbuilder.templates['edit/mintext']() %>\n<%= Formbuilder.templates['edit/maxvalue']() %>\n<%= Formbuilder.templates['edit/minvalue']() %>\n<%= Formbuilder.templates['edit/allowdecimals']() %>\n<%= Formbuilder.templates['edit/allowexponential']() %>\n<%= Formbuilder.templates['edit/decimalprecision']() %>\n<%= Formbuilder.templates['edit/decimalseparator']() %>\n<%= Formbuilder.templates['edit/negativetext']() %>",
...
(function() {
  Formbuilder.registerField('timefield', {
    order: 7,
    view: "<input type='text' class='rf-size-large' placeholder='<%= rf.get(Formbuilder.options.mappings.EMPTYTEXT) %>' />",
    edit: "\n<%= Formbuilder.templates['edit/partialtimefield']() %>\n<%= Formbuilder.templates['edit/partialrequiredfield']() %>\n<%= Formbuilder.templates['edit/partialvtype']() %>",
...
(function() {
  Formbuilder.registerField('datefield', {
    order: 8,
    view: "<input type='text' class='rf-size-large' placeholder='<%= rf.get(Formbuilder.options.mappings.EMPTYTEXT) %>' />",
    edit: "\n<%= Formbuilder.templates['edit/partialdatefield']() %>\n<%= Formbuilder.templates['edit/partialrequiredfield']() %>\n<%= Formbuilder.templates['edit/partialvtype']() %>",
...
(function() {
  Formbuilder.registerField('email', {
    order: 10,
    view: "<input placeholder='<%= rf.get(Formbuilder.options.mappings.EMPTYTEXT) %>' type='text' class='rf-size-large' />",
    edit: "<%= Formbuilder.templates['edit/emptytext']() %>",
...
(function() {
  Formbuilder.registerField('filefield', {
    order: 11,
    view: "<div class='width-full-inline'><div class='floating-box-input'><input type='text' class='rf-size-large' placeholder='<%= rf.get(Formbuilder.options.mappings.EMPTYTEXT) %>' /> </div><div class='floating-box-button'><button class='floating-box-button-config'><span class='symbol'><span class='glyphicon glyphicon-folder-open'></span></span> <%= rf.get(Formbuilder.options.mappings.BUTTONTEXT) %></button></div></div>",
    edit: "\n<%= Formbuilder.templates['edit/emptytext']() %>\n<%= Formbuilder.templates['edit/buttontext']() %>\n<%= Formbuilder.templates['edit/maxfilesize']() %>\n<%= Formbuilder.templates['edit/maxnumberfiles']() %>\n<%= Formbuilder.templates['edit/fileextensions']() %>",
...
(function() {
  Formbuilder.registerField('website', {
    order: 12,
    view: "<input type='text' class='rf-size-large' placeholder='http://<%= rf.get(Formbuilder.options.mappings.EMPTYTEXT) %>' />",
    edit: "\n<%= Formbuilder.templates['edit/emptytext']() %>\n<%= Formbuilder.templates['edit/blanktext']() %>\n<%= Formbuilder.templates['edit/allowblank']() %>\n<%= Formbuilder.templates['edit/hidden']() %>\n<%= Formbuilder.templates['edit/hideemptylabel']() %>\n<%= Formbuilder.templates['edit/hidelabel']() %>\n<%= Formbuilder.templates['edit/readonly']() %>\n<%= Formbuilder.templates['edit/value']() %>\n<%= Formbuilder.templates['edit/minlengthtext']() %>\n<%= Formbuilder.templates['edit/maxlengthtext']() %>\n<%= Formbuilder.templates['edit/validateblank']() %>\n<%= Formbuilder.templates['edit/min_max_length']() %>",
...
(function() {
  Formbuilder.registerField('photofield', {
    order: 14,
    view: "<div class='width-full-inline'><div class='floating-box-input'><input type='text' class='rf-size-large' placeholder='<%= rf.get(Formbuilder.options.mappings.EMPTYTEXT) %>' /> </div><div class='floating-box-button'><button class='floating-box-button-config'><span class='symbol'><span class='glyphicon glyphicon-folder-open'></span></span> <%= rf.get(Formbuilder.options.mappings.BUTTONTEXT) %></button></div></div>",
    edit: "\n<%= Formbuilder.templates['edit/emptytext']() %>\n<%= Formbuilder.templates['edit/buttontext']() %>\n<%= Formbuilder.templates['edit/maxfilesize']() %>\n<%= Formbuilder.templates['edit/maxnumberfiles']() %>",
...
(function() {
  Formbuilder.registerField('signaturefield', {
    order: 15,
    view: "<input type='text' class='rf-size-large' placeholder='<%= rf.get(Formbuilder.options.mappings.EMPTYTEXT) %>' />",
    edit: "\n<%= Formbuilder.templates['edit/emptytext']() %>\n<%= Formbuilder.templates['edit/blanktext']() %>\n<%= Formbuilder.templates['edit/allowblank']() %>\n<%= Formbuilder.templates['edit/hidden']() %>\n<%= Formbuilder.templates['edit/hideemptylabel']() %>\n<%= Formbuilder.templates['edit/hidelabel']() %>\n<%= Formbuilder.templates['edit/readonly']() %>\n<%= Formbuilder.templates['edit/value']() %>\n<%= Formbuilder.templates['edit/minlengthtext']() %>\n<%= Formbuilder.templates['edit/maxlengthtext']() %>\n<%= Formbuilder.templates['edit/validateblank']() %>\n<%= Formbuilder.templates['edit/min_max_length']() %>",
...
(function() {
  Formbuilder.registerField('qrcodefield', {
    order: 16,
    view: "<input type='text' class='rf-size-large' placeholder='<%= rf.get(Formbuilder.options.mappings.EMPTYTEXT) %>' />",
    edit: "\n<%= Formbuilder.templates['edit/emptytext']() %>\n<%= Formbuilder.templates['edit/blanktext']() %>\n<%= Formbuilder.templates['edit/allowblank']() %>\n<%= Formbuilder.templates['edit/hidden']() %>\n<%= Formbuilder.templates['edit/hideemptylabel']() %>\n<%= Formbuilder.templates['edit/hidelabel']() %>\n<%= Formbuilder.templates['edit/readonly']() %>\n<%= Formbuilder.templates['edit/value']() %>\n<%= Formbuilder.templates['edit/minlengthtext']() %>\n<%= Formbuilder.templates['edit/maxlengthtext']() %>\n<%= Formbuilder.templates['edit/validateblank']() %>\n<%= Formbuilder.templates['edit/min_max_length']() %>",
...
(function() {
  Formbuilder.registerField('barcodefield', {
    order: 17,
    view: "<input type='text' class='rf-size-large' placeholder='<%= rf.get(Formbuilder.options.mappings.EMPTYTEXT) %>' />",
    edit: "\n<%= Formbuilder.templates['edit/typecode']() %>\n<%= Formbuilder.templates['edit/emptytext']() %>\n<%= Formbuilder.templates['edit/blanktext']() %>\n<%= Formbuilder.templates['edit/allowblank']() %>\n<%= Formbuilder.templates['edit/hidden']() %>\n<%= Formbuilder.templates['edit/hideemptylabel']() %>\n<%= Formbuilder.templates['edit/hidelabel']() %>\n<%= Formbuilder.templates['edit/readonly']() %>\n<%= Formbuilder.templates['edit/value']() %>\n<%= Formbuilder.templates['edit/minlengthtext']() %>\n<%= Formbuilder.templates['edit/maxlengthtext']() %>\n<%= Formbuilder.templates['edit/validateblank']() %>\n<%= Formbuilder.templates['edit/min_max_length']() %>",
...
(function() {
  Formbuilder.registerField('labelfield', {
    order: 18,
...
(function() {
  Formbuilder.registerField('recordvideofield', {
    order: 19,
...
(function() {
  Formbuilder.registerField('recordvoicefield', {
    order: 20,
    view: "<label class='section-name'></label>\n",
    edit: "",
...
(function() {
  Formbuilder.registerField('gridfield', {
    order: 21,
    view: "<div id='contentGridfield'></div>",
    edit: "\n<%= Formbuilder.templates['edit/compositegrid']() %>",
    addButton: "<span class=\"symbol\"><span class=\"glyphicon glyphicon-th\"></span></span> Grid",
    defaultAttributes: function(attrs) {
      attrs.configuration.fieldLabel = 'Composite Grid';
    }
  });
}).call(this);
...
'\n' +
((__t = ( Formbuilder.fields[rf.get(Formbuilder.options.mappings.XTYPE)].edit({rf: rf}) )) == null ? '' : __t) +
'\n';
...
((__t = ( Formbuilder.options.mappings.FIELDLABEL )) == null ? '' : __t) +
'"></span>\n  <code class=\'field-type\' data-rv-text=\'model.' +
((__t = ( Formbuilder.options.mappings.XTYPE )) == null ? '' : __t) +
...
'\n  </div>\n' +
((__t = ( Formbuilder.templates['edit/hashtag']() )) == null ? '' : __t) +
((__t = ( Formbuilder.templates['edit/helptext']() )) == null ? '' : __t) + '\n</div>\n';
}
return __p
};
...
this["Formbuilder"]["templates"]["edit/optionsname"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
this["Formbuilder"]["templates"]["edit/optionscheckbox"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape, __j = Array.prototype.join;
function print() { __p += __j.call(arguments, '') }
with (obj) {
...
this["Formbuilder"]["templates"]["edit/optionscombo"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape, __j = Array.prototype.join;
function print() { __p += __j.call(arguments, '') }
with (obj) {
__p += '<div class=\'fb-edit-section-header\'>Options</div>\n\n';
__p += '\n\n<div class=\'option\' data-rv-each-option=\'model.' +
((__t = ( Formbuilder.options.mappings.OPTIONSCOMBO )) == null ? '' : __t) +
'\'>\n  <input type="text" data-rv-input="option:name" class=\'option-label-input\' />\n  <a class="js-add-option-combo ' +
((__t = ( Formbuilder.options.BUTTON_CLASS )) == null ? '' : __t) +'" title="Add Option"><i class=\'glyphicon glyphicon-plus-sign\'></i></a>\n  <a class="js-remove-option-combo ' +
((__t = ( Formbuilder.options.BUTTON_CLASS )) == null ? '' : __t) +'" title="Remove Option"><i class=\'glyphicon glyphicon-minus-sign\'></i></a>\n</div>\n\n';
__p += '\n\n<div class=\'fb-bottom-add\'>\n  <a class="js-add-option-combo ' +((__t = ( Formbuilder.options.BUTTON_CLASS )) == null ? '' : __t) +'">Add option</a>\n</div>\n';
}
return __p
};
...
__p += '<select data-rv-value="model.' +((__t = ( Formbuilder.options.mappings.SIZE )) == null ? '' : __t) +'">\n  <option value="small">Small</option>\n  <option value="medium">Medium</option>\n  <option value="large">Large</option>\n</select>\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.UNITS )) == null ? '' : __t) +'" />\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/emptytext"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" class="emptyText" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.EMPTYTEXT )) == null ? '' : __t) +'" />\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/hashtag"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" class="hashTag" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.HASHTAG )) == null ? '' : __t) +'" />\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/helptext"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" class="helpText" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.HELPTEXT )) == null ? '' : __t) +'" />\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/isevaluable"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
__p += '<div class=\'fb-edit-section-header\'>Emptytext</div>\n';
...
this["Formbuilder"]["templates"]["edit/blanktext"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" class="blankText" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.BLANKTEXT )) == null ? '' : __t) +'" />\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/allowblank"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<select class="allowBlank" data-rv-value="model.' +((__t = ( Formbuilder.options.mappings.ALLOWBLANK )) == null ? '' : __t) +'">\n  <option value="true">True</option>\n  <option value="false">False</option></select>\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<select class="hidde" data-rv-value="model.' +((__t = ( Formbuilder.options.mappings.HIDDEN )) == null ? '' : __t) +'">\n  <option value="true">True</option>\n  <option value="false">False</option></select>\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/hideemptylabel"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<select class="hideEmptyLabel" data-rv-value="model.' +((__t = ( Formbuilder.options.mappings.HIDEEMPTYLABEL )) == null ? '' : __t) +'">\n  <option value="true">True</option>\n  <option value="false">False</option></select>\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/hidelabel"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<select class="hideLabel" data-rv-value="model.' +((__t = ( Formbuilder.options.mappings.HIDELABEL )) == null ? '' : __t) +'">\n  <option value="true">True</option>\n  <option value="false">False</option></select>\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<select class="readOnly" data-rv-value="model.' +((__t = ( Formbuilder.options.mappings.READONLY )) == null ? '' : __t) +'">\n  <option value="true">True</option>\n  <option value="false">False</option></select>\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" class="value" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.VALUE )) == null ? '' : __t) +'" />\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/regex"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" class="regex" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.REGEX )) == null ? '' : __t) +'" />\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/regextext"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" id="regexText" class="regexText" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.REGEXTEXT )) == null ? '' : __t) +'" />\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/vtype"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<select class="vtype" data-rv-value="model.' +((__t = ( Formbuilder.options.mappings.VTYPE )) == null ? '' : __t) +'">\n  <option value="alpha">Alphabetic</option>\n  <option value="alphanum">Alphanumeric</option>\n</select>';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/vtypetext"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" class="vtypeText" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.VTYPETEXT )) == null ? '' : __t) +'" />\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/minlengthtext"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" class="minLengthText" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.MINLENGTHTEXT )) == null ? '' : __t) +'" />\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/maxlengthtext"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" class="maxLengthText" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.MAXLENGTHTEXT )) == null ? '' : __t) +'" />\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/validateblank"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<select class="validateBlank" data-rv-value="model.' +((__t = ( Formbuilder.options.mappings.VALIDATEBLANK )) == null ? '' : __t) +'">\n  <option value="true">True</option>\n  <option value="false">False</option>\n</select>\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/formatdatefield"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
this["Formbuilder"]["templates"]["edit/format"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
__p += '<div class=\'fb-edit-section-header\'>Format</div>\n';
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" class="format" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.FORMAT )) == null ? '' : __t) +'" />\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/invalidtext"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" class="invalidText" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.INVALIDTEXT )) == null ? '' : __t) +'" />\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/maxtext"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" class="maxText" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.MAXTEXT )) == null ? '' : __t) +'" />\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/mintext"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" class="minText" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.MINTEXT )) == null ? '' : __t) +'" />\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/maxvalue"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" class="maxValue" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.MAXVALUE )) == null ? '' : __t) +'" />\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/minvalue"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" class="minValue" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.MINVALUE )) == null ? '' : __t) +'" />\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/showtoday"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<select class="showToday" data-rv-value="model.' +((__t = ( Formbuilder.options.mappings.SHOWTODAY )) == null ? '' : __t) +'">\n  <option value="true">True</option>\n  <option value="false">False</option>\n</select>\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/submitformat"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<select class="submitFormat" data-rv-value="model.' +((__t = ( Formbuilder.options.mappings.SUBMITFORMAT )) == null ? '' : __t) +'">\n  <option value="true">True</option>\n  <option value="false">False</option>\n</select>\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/allowdecimals"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<select class="allowDecimals" data-rv-value="model.' +((__t = ( Formbuilder.options.mappings.ALLOWDECIMALS )) == null ? '' : __t) +'">\n  <option value="true">True</option>\n  <option value="false">False</option>\n</select>\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/allowexponential"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<select class="allowExponential" data-rv-value="model.' +((__t = ( Formbuilder.options.mappings.ALLOWEXPONENTIAL )) == null ? '' : __t) +'">\n  <option value="true">True</option>\n  <option value="false">False</option>\n</select>\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/decimalprecision"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" class="decimalPrecision" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.DECIMALPRECISION )) == null ? '' : __t) +'" />\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/decimalseparator"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<select class="decimalSeparator" data-rv-value="model.' +((__t = ( Formbuilder.options.mappings.DECIMALSEPARATOR )) == null ? '' : __t) +'">\n  <option value=".">Point</option>\n  <option value=",">Comma</option>\n</select>\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/negativetext"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" class="negativeText" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.NEGATIVETEXT )) == null ? '' : __t) +'" />\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/cols"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" class="cols" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.COLS )) == null ? '' : __t) +'" />\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/rows"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" class="rows" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.ROWS )) == null ? '' : __t) +'" />\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/pagesize"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" class="pageSize" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.PAGESIZE )) == null ? '' : __t) +'" />\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/typeahead"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<select class="typeAhead" data-rv-value="model.' +((__t = ( Formbuilder.options.mappings.TYPEAHEAD )) == null ? '' : __t) +'">\n  <option value="true">True</option>\n  <option value="false">False</option>\n</select>\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/forceselection"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<select class="forceSelection" data-rv-value="model.' +((__t = ( Formbuilder.options.mappings.FORCESELECTION )) == null ? '' : __t) +'">\n  <option value="true">True</option>\n  <option value="false">False</option>\n</select>\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/triggeraction"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<select class="triggerAction" data-rv-value="model.' +((__t = ( Formbuilder.options.mappings.TRIGGERACTION )) == null ? '' : __t) +'">\n  <option value="all">All</option>\n  <option value="query">Query</option>\n</select>\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/valuefield"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<select class="valueField" data-rv-value="model.' +((__t = ( Formbuilder.options.mappings.VALUEFIELD )) == null ? '' : __t) +'">\n  <option value="id">ID</option>\n  <option value="name">Name</option>\n</select>\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/displayfield"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<select class="displayField" data-rv-value="model.' +((__t = ( Formbuilder.options.mappings.DISPLAYFIELD )) == null ? '' : __t) +'">\n  <option value="id">ID</option>\n  <option value="name">Name</option>\n</select>\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/buttontext"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" class="buttonText" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.BUTTONTEXT )) == null ? '' : __t) +'" />\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/maxfilesize"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" class="maxFileSize" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.MAXFILESIZE )) == null ? '' : __t) +'" />\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/maxnumberfiles"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" class="maxNumberFiles" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.MAXNUMBERFILES )) == null ? '' : __t) +'" />\n';
__p += '</div>';
__p += '</div>';
}
return __p
...
this["Formbuilder"]["templates"]["edit/fileextensions"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
this["Formbuilder"]["templates"]["edit/typecode"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
this["Formbuilder"]["templates"]["edit/partialcheckbox"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<select class="checked" data-rv-value="model.' +((__t = ( Formbuilder.options.mappings.CHECKED )) == null ? '' : __t) +'">\n  <option value="true">True</option>\n  <option value="false">False</option></select>\n';
__p += '</div>';
__p += '</div>';
__p += '</div>';
__p += '<div class=\'input-line\'>';
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type=\'text\' class="boxLabel" data-rv-input=\'model.' +((__t = ( Formbuilder.options.mappings.BOXLABEL )) == null ? '' : __t) +'\' />\n';
__p += '</div>';
__p += '</div>';
__p += '</div>';
}
...
this["Formbuilder"]["templates"]["edit/partialrequiredfield"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type=\'text\' class="blankText" data-rv-input=\'model.' +((__t = ( Formbuilder.options.mappings.BLANKTEXT )) == null ? '' : __t) +'\' disabled/>\n';
__p += '</div>';
__p += '</div>';
__p += '</div>';
}
...
this["Formbuilder"]["templates"]["edit/partialregexp"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" class="regexText" id="inputRegexTextDatefield" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.REGEXTEXT )) == null ? '' : __t) +'" disabled/>\n';
__p += '</div>';
__p += '</div>';
__p += '</div>';
}
...
this["Formbuilder"]["templates"]["edit/partialvtype"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input class="vtypeText" type="text" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.VTYPETEXT )) == null ? '' : __t) +'" disabled/>\n';
__p += '</div>';
__p += '</div>';
__p += '</div>';
}
...
this["Formbuilder"]["templates"]["edit/partialformat"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<select class="format" data-rv-value="model.' +((__t = ( Formbuilder.options.mappings.FORMAT )) == null ? '' : __t) +'" >\n<option value="d/m/Y">DD/MM/YYYY</option>\n<option value="d/m/y">DD/MM/YY</option>\n<option value="d-m-Y">DD-MM-YYYY</option>\n<option value="d-m-y">DD-MM-YY</option>\n<option value="m/d/Y">MM/DD/YYYY</option>\n<option value="n/j/Y">M/D/YYYY</option>\n<option value="n/j/y">M/D/YY</option>\n<option value="m/j/y">MM/D/YY</option>\n<option value="n/d/y">M/DD/YY</option>\n<option value="m/j/Y">MM/D/YYYY</option>\n<option value="n/d/Y">M/DD/YYYY</option>\n<option value="m-d-y">MM-DD-YY</option>\n<option value="m-d-Y">MM-DD-YYYY</option>\n<option value="m/d">MM/DD</option>\n<option value="m-d">MM-DD</option>\n<option value="md">MMDD</option>\n<option value="mdy">MMDDYY</option>\n<option value="mdY">MMDDYYYY</option>\n<option value="d">DD</option>\n<option value="Y-m-d">YYYY-MM-DD</option>\n<option value="n-j">M-D</option>\n<option value="n/j">M/D</option>\n</select>\n';
__p += '</div>';
__p += '</div>';
__p += '</div>';
__p += '<div class=\'input-line\'>';
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input class="invalidText" type="text" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.INVALIDTEXT )) == null ? '' : __t) +'" disabled/>\n';
__p += '</div>';
__p += '</div>';
__p += '</div>';
}
...
this["Formbuilder"]["templates"]["edit/partialtimefield"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
/************************************************************************/
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type=\'text\' class="emptyText" id="inputEmptyTextTimefield" data-rv-input=\'model.' +((__t = ( Formbuilder.options.mappings.EMPTYTEXT )) == null ? '' : __t) +'\' />\n';
__p += '</div>';
__p += '</div>';
/************************************************************************/
__p += '<div class=\'fb-edit-section-header\'>Hidden</div>';
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.SUBMITFORMAT )) == null ? '' : __t) +'" disabled/>\n';
__p += '</div>';
__p += '</div>';
/************************************************************************/
/**Regular Expression**/
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" class="regex" id="inputRegexTimefield" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.REGEX )) == null ? '' : __t) +'" />\n';
__p += '</div>';
__p += '</div>';
__p += '</div>';
__p += '<div class=\'input-line\'>';
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" class="regexText" id="inputRegexTextTimefield" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.REGEXTEXT )) == null ? '' : __t) +'" disabled/>\n';
__p += '</div>';
__p += '</div>';
__p += '</div>';
...
this["Formbuilder"]["templates"]["edit/partialdatefield"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
/************************************************************************/
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type=\'text\' class="emptyText" id="inputEmptyTextDatefield" data-rv-input=\'model.' +((__t = ( Formbuilder.options.mappings.EMPTYTEXT )) == null ? '' : __t) +'\' />\n';
__p += '</div>';
__p += '</div>';
/************************************************************************/
__p += '<div class=\'fb-edit-section-header\'>Hidden</div>';
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<input type="text" class="regex" id="inputRegexDatefield" data-rv-input="model.' +((__t = ( Formbuilder.options.mappings.REGEX )) == null ? '' : __t) +'" disabled/>\n';
__p += '</div>';
__p += '</div>';
__p += '</div>';
__p += '<div class=\'input-line\'>';
...
__p += '<div class="fb-common-wrapper">';
__p += '<div class="fb-label-description">';
__p += '<select class="format" id="selectFormatDatefield" data-rv-value="model.' +((__t = ( Formbuilder.options.mappings.FORMAT )) == null ? '' : __t) +'" >\n<option value="d/m/Y">DD/MM/YYYY</option>\n<option value="d/m/y">DD/MM/YY</option>\n<option value="d-m-Y">DD-MM-YYYY</option>\n<option value="d-m-y">DD-MM-YY</option>\n<option value="m/d/Y">MM/DD/YYYY</option>\n<option value="n/j/Y">M/D/YYYY</option>\n<option value="n/j/y">M/D/YY</option>\n<option value="m/j/y">MM/D/YY</option>\n<option value="n/d/y">M/DD/YY</option>\n<option value="m/j/Y">MM/D/YYYY</option>\n<option value="n/d/Y">M/DD/YYYY</option>\n<option value="m-d-y">MM-DD-YY</option>\n<option value="m-d-Y">MM-DD-YYYY</option>\n<option value="m/d">MM/DD</option>\n<option value="m-d">MM-DD</option>\n<option value="md">MMDD</option>\n<option value="mdy">MMDDYY</option>\n<option value="mdY">MMDDYYYY</option>\n<option value="d">DD</option>\n<option value="Y-m-d">YYYY-MM-DD</option>\n<option value="n-j">M-D</option>\n<option value="n/j">M/D</option>\n</select>\n';
__p += '</div>';
__p += '</div>';
__p += '</div>';
__p += '<div class=\'input-line\'>';
...
this["Formbuilder"]["templates"]["edit/compositegrid"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape, __j = Array.prototype.join;
function print() { __p += __j.call(arguments, '') }
with (obj) {
__p += '<div class=\'fb-bottom-add\' >\n';
__p += '<button id="buttonModalFormbuilder" value="" data-toggle="modal" data-target="#modalFormbuilder" class="js-save-form fb-button" title="Fields Settings"><i class=\'glyphicon glyphicon-plus-sign\'></i></span></span> Fields Settings</button>\n';
__p += '</div>\n';
}
return __p
};
...
this["Formbuilder"]["templates"]["newpage"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
__p +=
...
this["Formbuilder"]["templates"]["partials/custome_field"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape;
with (obj) {
...
'\n    ' +
//((__t = ( Formbuilder.templates['partials/custome_field']() )) == null ? '' : __t) +
'\n  </div>\n</div>';
...
'\n\n  ' +
((__t = ( Formbuilder.fields[rf.get(Formbuilder.options.mappings.XTYPE)].view({rf: rf}) )) == null ? '' : __t) +
'\n\n  ' +
...
with (obj) {
__p += '<label>\n  <span>' +
((__t = ( Formbuilder.helpers.simple_format(rf.get(Formbuilder.options.mappings.FIELDLABEL)) )) == null ? '' : __t) +
'\n  ';
